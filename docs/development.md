...
| `go run ./scripts/gates api-coverage` | Every method the two discovery documents publish, and every one of the Sheets `batchUpdate` union's request kinds, is either used — naming the code that implements it — or written off with a reason. It holds `testdata/api-surface.json`, `testdata/api-coverage.tsv` and the code to each other, offline, so a capability Google adds fails the build. One `*` row may stand for the rest of an API and may only ever excuse: Drive publishes sixty-four methods and this server calls three, and §1 puts the rest in another server. |
| `go run ./scripts/gates api-diff` | Refetches the discovery documents, rewrites the snapshot and reports NEW, GONE and CHANGED with verb and path — a method that keeps its name and moves is a break a list of names cannot show. **Manual**, because a gate that fails when Google is slow is one people learn to re-run until it passes. What CI holds is the snapshot this writes. |
